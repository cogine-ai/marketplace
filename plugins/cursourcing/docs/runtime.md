...
- `wait` returns immediately for an ended turn, failure, interruption, cancellation, or pending request, not ordinary tool/progress events. Its compact default includes status and unread completed replies without event pages or native paths. Pass `next_cursor` into `after_cursors` to acknowledge completed results; unresolved requests and a lost runtime remain actionable. Timeouts return small status records without stopping execution. The default and maximum are 120 seconds; `.mcp.json` allows 150 seconds per MCP tool call. Manually configured hosts need a matching deadline or an explicitly shorter inner wait. A yielding wrapper can continue the same pending call; its budget and progress-update requirements remain host-controlled. Increasing the inner timeout alone does not guarantee fewer model responses. See [host waiting examples](../skills/cursourcing/references/waiting.md).
...
- Failed initialization/execution stays `cleaning` until process cleanup settles, then returns the original error with `failure_phase` and `cleanup_status`. Repeated closes join one promise. Cleanup allows EOF, sends TERM after 1.5 seconds and KILL after 4 seconds, and returns within a 6-second budget. On POSIX this targets the runtime's dedicated CLI process group, including descendants left by an exited wrapper; Windows targets the direct child. This is not containment of processes that deliberately detach from that group.
- Unconfirmed cleanup returns `cleanup_status: "blocked"` and retains the process reference. Sending, resuming, replaying history, and starting a replacement in the same workspace are blocked until that owned process is confirmed gone; this guard survives runtime restart. Ordinary running timeouts do not trigger recovery or retries. No prompt is replayed automatically.
- Execution records mark the spawn window before launch and save the CLI process/group reference immediately after it. If the owner dies, recovery, history replay and replacement work in that workspace are blocked while the previous execution is alive or unconfirmed. Eligibility is checked again after acquiring a stale lease. Persisted process references are only checked for liveness, never used to signal a process from another runtime; PID reuse may conservatively block recovery. Legacy records use retained journal PID evidence when available; active legacy records without sufficient evidence require inspection rather than blind takeover. Normal tasks with live owners can still run concurrently.
- Failed, interrupted, cancelled and cleaning statuses include `recovery: {session_available, can_resume, reason?}`. Reasons distinguish no session, incomplete cleanup, surviving/unknown previous execution, another live owner, and an already connected session. A connected session needs no reload. `can_resume` describes current eligibility, not a guarantee that retrying will succeed; every action checks live state again. Running/idle status remains compact without these fields.
- Runtime shutdown shares one promise across stdin closure and repeated signals. Because hosts can force-kill an exiting MCP server, shutdown and aborted replay send TERM immediately, KILL after 250 ms, and use a 1.5-second cleanup budget. Execution and replay clients drain concurrently before ownership leases are released. An uncatchable server kill can still prevent graceful cleanup; this is not an OS-level parent-death guarantee.
