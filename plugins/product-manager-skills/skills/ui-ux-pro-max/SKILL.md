---
name: ui-ux-pro-max
description: Search the bundled UI/UX database for product-specific styles, palettes, typography, accessibility, motion, charts, and stack guidance. Use only when explicitly requested.
...
When explicitly requested, use this skill for tasks involving **UI structure, visual design decisions, interaction patterns, or user experience quality control**: designing new pages, creating/refactoring UI components, choosing color/typography/spacing/layout systems, reviewing UI for UX/accessibility/consistency, implementing navigation/animation/responsive behavior, or improving perceived quality and usability.
...
The search script lives inside this skill's own directory, not the project directory. Resolve `UI_UX_SKILL_DIR` to the absolute directory containing this `SKILL.md`, then invoke the script by full path:

```bash
python3 "$UI_UX_SKILL_DIR/scripts/search.py" "<query>" --domain <domain>
```
...
```bash
python3 "$UI_UX_SKILL_DIR/scripts/search.py" "<product_type> <industry> <keywords>" --design-system [-p "Project Name"]
```
...
**Example:**
```bash
python3 "$UI_UX_SKILL_DIR/scripts/search.py" "beauty spa wellness service" --design-system -p "Serenity Spa"
```

### Step 2b: Persist Design System (Master + Overrides Pattern)

First inspect a non-persisted result and verify its fit using the Query Contract. Only then, to save it for retrieval across sessions, add `--persist` **and always pass `--output-dir` pointed at the project root** — without it, files are written relative to whatever directory the tool happens to run from:

```bash
python3 "$UI_UX_SKILL_DIR/scripts/search.py" "<query>" --design-system --persist -p "Project Name" --output-dir "<project-root>"
```
...
```bash
python3 "$UI_UX_SKILL_DIR/scripts/search.py" "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```bash
python3 "$UI_UX_SKILL_DIR/scripts/search.py" "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```

### Step 3: Supplement with Detailed Searches (as needed)

```bash
python3 "$UI_UX_SKILL_DIR/scripts/search.py" "<keyword>" --domain <domain> [-n <max_results>]
```
...
```bash
python3 "$UI_UX_SKILL_DIR/scripts/search.py" "<keyword>" --stack <stack>
```
...
python3 "$UI_UX_SKILL_DIR/scripts/search.py" "AI search tool modern minimal" --design-system -p "AI Search"

# Step 3: supplement
python3 "$UI_UX_SKILL_DIR/scripts/search.py" "keyboard focus modal" --domain ux

# Step 4: stack guidelines
python3 "$UI_UX_SKILL_DIR/scripts/search.py" "suspense streaming bundle" --stack nextjs
```
