---
name: ui-ux-pro-max
description: Search a bundled UI/UX database for product-specific styles, palettes, typography, accessibility, motion, charts, and stack guidance. Use only when explicitly requested or when a concrete design-system lookup is needed for web or mobile UI.
---
...
The search script lives inside this skill's own directory, not the project directory. Resolve `UI_UX_SKILL_DIR` to the directory containing this `SKILL.md`, then invoke it by full path:
...
# Step 3: supplement
python3 "$UI_UX_SKILL_DIR/scripts/search.py" "search loading animation" --domain ux
