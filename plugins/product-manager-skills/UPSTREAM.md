...
`to-spec` and `to-tickets` are refreshed from Matt `3cca18b368ae95cdbdebbff572ccafa662551015`. Preserve short explicit triggers, native host policies, repository tracker discovery, safe local fallback, and existing-label-only publication. The other Matt-derived skills, `grilling` and `prototype`, retain their existing implementations and source records.
