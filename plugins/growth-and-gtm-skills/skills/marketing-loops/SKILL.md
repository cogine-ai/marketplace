...
description: Design a bounded, recurring marketing workflow with triggers, cadence, durable state, guardrails, human checkpoints, and stopping conditions. Use for always-on reviews, alerts, refreshes, and other agent-run marketing loops.
---
...
This is the operational layer for work worth repeating. Strategy skills decide
what to try; loops define what to keep doing on a cadence and wire the other
skills together.
...
- **Codex** — use Automations for fixed schedules or recurring monitors when available.
- **Claude Code** — use the available loop, wakeup, or cron mechanism.
...
- **content-strategy** and **acquisition-channels** — choose what to try; loops operationalize what is worth repeating.
- **growth-experiments** — the experimentation loop specifically (hypothesis → test → promote winner → repeat).
- **analytics** — most loops read from analytics to decide whether to act.
- Channel skills such as `search-visibility`, `content-strategy`,
  `copywriting`, `cro`, `onboarding`, `churn-prevention`, and `pricing`
  provide the loop bodies.
