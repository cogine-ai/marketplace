...
Search several angles, not one query repeated. Adapt wording to how the audience actually talks by mining their vocabulary from public organic content first.
...
Use available web search and browser tools to constrain results by date, open the
original public page, and verify the full context rather than qualifying from a
search snippet. For a broad market, sweep multiple source types and challenge
each apparent signal with contradictory evidence.
...
Never claim familiarity you don't have, never fabricate personal details, and never auto-send: no messages, connects, follows, comments, form submissions, or CRM records unless the user separately authorizes that action.
...
If the user asks for a shareable standalone HTML report, keep it self-contained and escape every external value.
